...
public class QueryMdSystem extends HttpServlet {
	...
	protected void doGet(final HttpServletRequest request,
		...
		try {
			...
			final MdSystemQueryHandler mdQueryHandler = MdSystemQueryHandler
					.getInstance();
			mdQueryHandler.init();
			logger.info("******************** ");

			if (conceptSearch != null && conceptSearch.equals("true")) {
				handleConceptQuery(request, response, out, logger, query, outputFormat,
						begin, mdQueryHandler);
			}

			if (detailedSearch != null && detailedSearch.equals("true")) {
				handleDetailedSearch(logger, begin, outputFormat, query, request,
						response, out);
			}
	...
	private void handleDetailedSearch(final Logger logger, final Date begin,
			final String outputFormat, final String query,
			final HttpServletRequest request, final HttpServletResponse response,
			final PrintWriter out) {
		logger.info("detailed Search");
		final ISparqlAdapter adapter = useFuseki();
		// final ISparqlAdapter adapter = useJena();
		final HitGraphContainer resultContainer = adapter.buildSparqlQuery("+"
				+ query);
		...
		if (outputFormat.equals("html")) {
			...
			htmlStrBuilder.append("\n\t\t</table>");
			htmlStrBuilder.append("\n\t\t<p><strong>Search term:</strong> " + query
					+ "</p>");
			htmlStrBuilder.append("\n\t\t<p><strong>Number of hits:</strong> "
					+ resultContainer.size() + "</p>");
			htmlStrBuilder.append("\n\t\t<ul>");

			int counter = 0;
			for (final HitGraph hitGraph : resultContainer.getAllHits()) {
				htmlStrBuilder.append("\n\t\t\t<li><strong>QueryHit #" + (++counter)
						+ "</strong></li>");
				htmlStrBuilder.append("\n\t\t\t\t<li><ul>");
				htmlStrBuilder
						.append("\n\t\t\t\t\t<li><strong>NamedGraphUrl:</strong> "
								+ hitGraph.getNamedGraphUrl() + "</li>");
				htmlStrBuilder.append("\n\t\t\t\t\t<li><strong>Average score:</strong>"
						+ hitGraph.getAvgScore() + "</li>");
				htmlStrBuilder.append("\n\t\t\t\t\t<li><strong>Highest score:</strong>"
						+ hitGraph.getHighestScore() + "</li>");
				for (final HitStatement hitStatement : hitGraph.getAllHitStatements()) {
					htmlStrBuilder.append("\n\t\t\t\t<li><ul>");
					htmlStrBuilder.append("\n\t\t\t\t\t\t<li><strong>Subject:</strong> "
							+ hitStatement.getSubject() + "</li>");
					htmlStrBuilder
							.append("\n\t\t\t\t\t\t<li><strong>Predicate:</strong> "
									+ hitStatement.getPredicate() + "</li>");
					htmlStrBuilder.append("\n\t\t\t\t\t\t<li><strong>Literal:</strong> "
							+ hitStatement.getObject() + "</li>");
					htmlStrBuilder
							.append("\n\t\t\t\t\t\t<li><strong>Parent subject:</strong> "
									+ hitStatement.getSubjParent() + "</li>");
					htmlStrBuilder
							.append("\n\t\t\t\t\t\t<li><strong>Parent predicate:</strong> "
									+ hitStatement.getPredParent() + "</li>");
					htmlStrBuilder.append("\n\t\t\t\t\t\t<li><strong>Score:</strong> "
							+ hitStatement.getScore() + "</li>");
					htmlStrBuilder.append("\n\t\t\t\t</li></ul>");
				}
		 ...
		 */
			...
			final String jsonString = JSONValue.toJSONString(jsonEncoder
					.getJsonObject());
	 ...
	 */
	private void handleConceptQuery(final HttpServletRequest request,
			final HttpServletResponse response, final PrintWriter out,
			final Logger logger, final String query, final String outputFormat,
			final Date begin, final MdSystemQueryHandler mdQueryHandler) {
		final ArrayList<ConceptQueryResult> conceptHits = mdQueryHandler
				.getConcept(query);
		final Date end = new Date();
		final long elapsedTime = end.getTime() - begin.getTime();
		logger.info("elapsedTime : " + elapsedTime);
		...
		if (outputFormat.equals("json") && conceptHits != null) {
			...
			jsonEncoder.putStrings(SEARCH_TERM, query);
			jsonEncoder
					.putStrings(NUMBER_OF_HITS, String.valueOf(conceptHits.size()));
			...
			for (int i = 0; i < conceptHits.size(); i++) {
				...
				for (final String s : keys) {
					logger.info("concepts.get(i).getValue(s) : "
							+ conceptHits.get(i).getValue(s));
	...
	public static ISparqlAdapter useJena() {
		...
		try {
			jenamain.initStore();
			final IQueryStrategy<Map<URL, ResultSet>> queryStrategy = new QueryStrategyJena(
					jenamain);
